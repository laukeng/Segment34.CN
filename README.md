# Segment34.CN
修改自Segment34 MkII表盘([https://github.com/ludw/Segment34mkII](https://github.com/ludw/Segment34mkII))，感谢原作者开源！添加了农历和二十四节气显示，顺便对设置菜单和天气也做了汉化。由于没有制作点阵风格的汉字字体，当显示农历时请将时钟上方的字体设为“线条”（默认值），以确保能正常显示汉字。

<img src="screenshot.jpg" alt="Screenshot of the watchface" width="400">

## 更新日志 
2026年9月12日 v2.5.5版：  
在设置菜单里增加了月相阴影显示开关。  

2026年9月8日 v2.5.0版：  
1.新增对Fenix 9系列的支持；  
2.重新设计了更精细的月相图标。  

2026年6月1日 v2.4.5版：  
修复了湿度缓存过期之后导致崩溃的问题  

2026年4月7日 v2.4.4版：  
修正了原版存在的切换主题时不显示提示的问题。  

2026年4月4日 v2.4.3版：  
1.修复了上一个版本优化农历模块时出现的一处错误；  
2.底部默认布局改成了334，以免有些用户不会改。    

2026年4月2日 v2.4版：  
1.进一步优化农历模块以节省运行内存；  
2.修复了有些设备上电量条形图显示异常的问题。  

2026年4月1日 v2.3.3版：  
1.给身体压力值增加了缓存，解决了压力有时候不显示的问题；  
2.调整了语言顺序，解决某些手机上设置页面不显示中文的问题。

2026年3月30日 v2.3.2版：  
1.增加了一些汉字以修复训练状态显示方框的Bug；  
2.修复了AOD时钟下方星期显示为方框的Bug；  
3.修复了顶部和底部显示温度时摄氏度图标显示为方框的Bug。  

2026年3月28日 v2.2.1版：
1.修复了摄氏度图标跟右边逗号太近的问题；
2.修改了设置页面中的几处翻译。

2026年3月28日 v2.2版：
1.移除了几款不支持的设备：F6S、F6SP、245、945等，这几款设备可以试一下精简版；
2.增加了一种日期格式：“YYYY-MM-DD 星期”。 

2026年3月27日 v2.1版：  
时钟上面两行和下面都可以设置为显示农历；  

2026年3月26日 v2.0版：  
完成了所有汉化，包括天气和星期还有摄氏度图标；  

2026年3月25日 v1.4：  
1.将日期里面的星期显示改成了中文；  

2026年3月24日 v1.3：  
1.修复了农历字体缺失的几个字符；  
2.同步更新代码至Segment34 MkII 4.6.2；  
 
2026年3月23日 v1.2：  
1.重写了农历算法，以适应老款设备；  
2.为AMOLED屏设计了大号的字体；  
3.在设置里面增加了农历的选项菜单；  